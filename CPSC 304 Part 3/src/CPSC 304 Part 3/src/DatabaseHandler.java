    ...
    public Reservation createReservation(int dlicense, String carType, Connection con, java.util.Date fromDate, java.util.Date toDate) throws NoSuchCustomerException, NoCarAvailableException {
        ...
        try {
            ...
            if (!rs.next()) {
                ...
                con.commit();
                ResultSet vt = statement.executeQuery("SELECT vlicense FROM Vehicle WHERE vtname =" + quote(carType));
...
//                throw new NoCarAvailableException();
    ...
    public ArrayList<Vehicle> getDailyRentals(Connection con){
        ...
        try {
            Statement statement = con.createStatement();
            ResultSet rs = statement.executeQuery("SELECT * FROM Rent r, Vehicle v WHERE r.vlicense = v.vlicense AND r.fromDate = '12-DEC-19' ORDER BY v.location, v.vtname");
            while (rs.next()){
